...
contract PoapContract is ERC1155, AccessControl, ERC1155Supply {
    ...
    mapping(address => PoapId[]) public poapsByAccount;
    ...
    function initialize(string memory _uri) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setURI(_uri);
    }
    ...
    function mint(address _account, uint256 _eventId) public onlyRole(MINTER_ROLE) {
    ...
    function setEventURI(uint256 eventId, string memory newURI) public onlyRole(URI_SETTER_ROLE) {
    ...
    {
        // Implement batch minting logic if needed
    }
